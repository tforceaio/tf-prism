...
const configFileName = "prism.yaml"
...
func InitKoanf(useFS bool) (*RootConfig, error) {
	...
	if isPortable {
		...
		home := os.Getenv("HOME")
		configFile = filepath.Join(home, ".config", "tfprism", configFileName)
	} else if runtime.GOOS == "windows" {
		appData := os.Getenv("APPDATA")
		configFile = filepath.Join(appData, "TFprism", configFileName)
	}
...
func IsPortable() bool {
	exec, _ := os.Executable()
	portableFile := filepath.Join(filepath.Dir(exec), "prism.portable")
...
func configFromEnv(k *koanf.Koanf) (*koanf.Koanf, error) {
	err := k.Load(env.Provider("TFPRISM_", ".", func(s string) string {
		return strings.Replace(
			strings.ToLower(
				strings.TrimPrefix(s, "TFPRISM_")), "_", ".", -1)
	}), nil)
	if err != nil {
		return k, err
	}
